...
export function useDagDragAndDrop(options: Options) {
  ...
  const projectCode = Number(route.params.projectCode) ||Number(route.query.projectCode)
  ...
  function onDrop(e: DragEvent) {
    ...
    console.log(e,readonly.value,"readonly.value",projectCode);
    if (dragged.value && graph.value && projectCode) {
      ...
      genTaskCodeList(genNums, projectCode).then((res) => {
        ...
        const [code] = res
        console.log(code, type, { x: x - eX, y: y - eY });
