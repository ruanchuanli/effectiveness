...
import { render, h, ref, toRef } from 'vue'
...
export function useNodeStatus(options: Options) {
  ...
  const processInstanceState = ref()
  ...
  const refreshTaskStatus = () => {
    ...
    queryTaskListByProcessId(instanceId, projectCode).then((res: any) => {
      ...
      processInstanceState.value = parseInt(res.processInstanceState.match(/code=(\d+)/)[1]);
