...
import { determineIfConnectionIsPossible } from '@/service/modules/assessmentPlan'
interface Options {
...
export function useCanvasInit(options: Options) {
  ...
  onMounted(() => {
    ...
    const that =  graph.value
    // Make sure the edge starts with node, not port
    graph.value.on('edge:connected', ({ isNew, edge }) => {
      
      console.log(isNew, edge, 'isNew, edge');
      console.log(edge.source, '-----------------', edge.target, edge.getSourceNode(), '----', edge.getTargetNode());
      const getSource = edge.getSourceNode()?.getData()
      const getTarget = edge.getTargetNode()?.getData()
      determineIfConnectionIsPossible({ operatorIdA: Number(getSource.operatorId), operatorIdB: Number(getTarget.operatorId) }).then((res: any) => {
        console.log(res);
        if (isNew) {
