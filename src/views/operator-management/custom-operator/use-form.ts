...
import type { ICustomOperator } from '@/service/modules/custom-operator/types'
...
export function useForm(id?: number) {
  ...
  const initialValues = {
    rawScriptShiLi:
      "source activate && conda activate op && python ${filePath} \"{'data1':${input1},'data2':${input2}\"\"['output1']\"",
    rawScriptShiLi2:
      '（1）source activate && conda activate op && python ${filePath} 此部分代码固定，请谨慎修改',
    rawScriptShiLi3:
      "（2）\"{'data1':${input1},'data2':${input2}\"\"['output1']\" 部分为参数信息,data1是入参json，需和代码中入参json名称相同，" +
    ...
    operatorDesc: '',
    rawScript:
      "python3 ${filePath} ${SourceInput_SUBFLOWUUID} ${EvaluationObjs}",
    fileName: '',
    file: null,
    filePath: '',
    inputParams: [],
  ...
  const setFieldsValue = (values: ICustomOperator) => {
    state.detailForm = {
      ...state.detailForm,
      ...values
    }
