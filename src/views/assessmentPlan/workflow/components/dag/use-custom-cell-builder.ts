...
import { X6_NODE_NAME, X6_EDGE_NAME, X6_NODE_SUB_PROCESS_NAME } from './dag-config'
...
export function useCustomCellBuilder() {
  ...
  function buildNode(
    ...
    return {
      id: id,
      shape: type == 'SUB_PROCESS' ? X6_NODE_SUB_PROCESS_NAME : X6_NODE_NAME,
      x: coordinate.x,
      y: coordinate.y,
      data: {
        taskType: type,
      ...
      attrs: {
        ...
        rect: {
          fill: type === 'SUB_PROCESS' ? '#40E0D0' : flag === 'NO'  ? '#f3f3f5' : '#ffffff'
        }
      }
    }
  }
