...
import { TaskDefinition, WorkflowDefinition, WorkflowInstance } from './types'
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    provide('processDefinition', processDefinition)
    ...
    const weightDetail = ref<Record<string, any>>({})

    const updateWeightDetail = () => {
      if (labelObj.value?.name) {
        weightDetail.value.weightList = []
        getWeightDetail(route.params.id as string, labelObj.value?.code).then(
          (res: any) => {
            weightDetail.value = res
            weightDetail.value.weightType = res.weightType || 1
            console.log(weightDetail.value);
          }
        )
      }
    }
    ...
    return () => (
      ...
      >
        <DagToolbar definition={props.definition} />
        <div class={Styles.content}>
          ...
          <RightWeight
            onStepOver={stepOver}
            weightDetail={weightDetail.value}
            labelData={labelObj.value}
            treeData={processDefinition.value.taskDefinitionList}
            relationship={processDefinition.value.processTaskRelationList}
          />
        </div>
        <DagAutoLayoutModal
