...
import { remove, cloneDeep, keyBy } from 'lodash'
...
export function useTaskEdit(options: Options) {
  ...
  const labelObj = ref<Record<string, any>>({})
  ...
  function labelShow(code: number|undefined) {//显示label信息
    if (code) {
      const { taskDefinitionList, processTaskRelationList } = processDefinition.value
      const taskMap = keyBy(taskDefinitionList, 'code')
      const relationMap = keyBy(processTaskRelationList, 'postTaskCode')
      const definition = taskMap[code]
      const relation = relationMap[code]
      if (definition) {
        labelObj.value = { ...definition, relation }
      }
